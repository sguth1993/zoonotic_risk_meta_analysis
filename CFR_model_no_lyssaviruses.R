...
#remove bat lyssaviruses
dat.mort <- dat.mort[dat.mort$SppName_ICTV_MSL2018b!="Australian bat lyssavirus" &
                       dat.mort$SppName_ICTV_MSL2018b!="Duvenhage lyssavirus" &
                       dat.mort$SppName_ICTV_MSL2018b!="European bat 1 lyssavirus" &
                       dat.mort$SppName_ICTV_MSL2018b!="European bat 2 lyssavirus" &
                       dat.mort$SppName_ICTV_MSL2018b!="Rabies lyssavirus" &
                       dat.mort$SppName_ICTV_MSL2018b!="Irkut lyssavirus", ]
...
dat.mort$CFR <- (dat.mort$CFR*(80-1) + 0.5)/80
...
#record effect sizes
effects_dat <- plot.gam(gam_mort, residuals=TRUE, select = 4)[[4]]
...
effect_sizes <- rbind(effects_hOrd, effects_spill, effects_vec)
write.csv(effect_sizes,"Extension/code_and_data/data/effect_sizes/CFR_effects_no_lyssa.csv")
...
Fig1 <- cowplot::plot_grid(mort_models, hOrder, pub_count, binary,
                           ncol=4, nrow=1, align = "h", 
                           labels = c( "A", "B", "C", "D"),
                           label_size = 20)

# Fig1b <- cowplot::plot_grid(binary, phylo_dist,  sp_rich,
...
gam.dat <- dat.mort %>%
  group_by(hOrder) %>%
  summarise(phylo_dist = median(phylo_dist),
            ReservoirNspecies = median(ReservoirNspecies),
...
#predicted CFR for bats?
gam.dat$CFR[gam.dat$hOrder=="CHIROPTERA"]
...
ggsave(file = "Extension/code_and_data/figures/Figure_S5.pdf",
       units="mm",
       width=80,
       height=80,
       scale=2,
