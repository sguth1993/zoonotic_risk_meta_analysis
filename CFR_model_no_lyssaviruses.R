...
dat.mort$CFR <- (dat.mort$CFR*(79-1) + 0.5)/79
...
plotData_phylo_dist <- get_partial_effects_continuous(gam_mort, 
                                                      var = 'phylo_dist',
                                                      resolution = 1, 
                                                      seWithMean = TRUE)
...
phylo_dist <- ggplot(plotData_phylo_dist$effects, aes(x = phylo_dist, y = y, colour = IsSignificant, fill = IsSignificant)) +
  ...
  scale_fill_manual(values = c(No = 'grey60', Yes = 'mediumorchid4'), guide = F) +
  scale_y_continuous(labels = function(x) sprintf('%1.1f', x) ) +
  labs(x = "phylogenetic distance from Primates", y = 'Effect on CFR in humans') +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
...
plotData_sp_rich <- get_partial_effects_continuous(gam_mort,
...
Fig1a <- cowplot::plot_grid(mort_models, hOrder, pub_count,
