...
dat.mort$CFR_country_specific <- (dat.mort$CFR_country_specific*(119-1) + 0.5)/119

#let's see if GDPPC is significant 
#### model selection ####
Terms <- list(
  GDP = c(NA, "s(GDP, k=7, bs='tp')"),
...
#vector-borne
plotData <- get_partial_effects_binary_single(fit = country_gam_mort, 
                                              var = 'IsVectorBorne',
                                              seWithMean = TRUE, 
                                              fixedEffect = FALSE,
...
pub_count <- ggplot(plotData_pub_count$effects, aes(x = VirusSppPublicationCount, y = y, colour = IsSignificant, fill = IsSignificant)) +
  ...
  labs(x = "virus species publication count", y = 'Effect on country-specific\n CFR in humans') +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
...
#GDPPC
plotData_GDP <- get_partial_effects_continuous(country_gam_mort, 
                                                     var = 'GDP')

GDP <- ggplot(plotData_GDP$effects, aes(x = GDP, y = y, colour = IsSignificant, fill = IsSignificant)) +
  coord_cartesian(clip="off") +
  geom_hline(yintercept = 0, linetype = 2, colour = 'grey20') +
  geom_jitter(aes(y = Residual), alpha = 0.6, size = 0.8,
              width = 3, height = 0,
              data = plotData_GDP$partialResiduals) +
  geom_ribbon(aes(ymin = ylower, ymax = yupper, colour = NA), alpha = 0.5) +
  ...
  scale_fill_manual(values = c(No = 'grey60', Yes = 'mediumorchid4'), guide = F) +
  scale_y_continuous(labels = function(x) sprintf('%1.1f', x) ) +
  labs(x = "country GDP per capita", y = 'Effect on country-specific\n CFR in humans') +
  theme_bw() +
  theme(axis.title = element_text(size = 12),
...
Fig1 <- cowplot::plot_grid(CFR_country_models, hOrder_country,vFamily_country,
                            GDP, vector, pub_count,
